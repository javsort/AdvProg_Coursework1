public class MazeSquare extends GameSquare
{
	...
	public void leftClicked()
	{
		...
		this.setHighlight(true);	// Highlight the target
	...
	{
		// Find a target to compare and set current square
		targetSq = findTarget();
		currentSq = this;

		// If target is found, start search
		if(targetSq != null && targetSq.target){
			System.out.println("Source has been set at: (" + currentSq.getXLocation() + "," + currentSq.getYLocation() + ").\n");

			// Create a new PathFinder object and call search
	...
	public void reset(int n)
	{
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				...
				currentSq.setHighlight(false);				// Clear the highlight
			}
		}
	}

	// Method to find target in the maze
	public MazeSquare findTarget(){
		...
		// If there's no target, set target first
		reset(0);
		return null;
	}

	// Setter for visited squares
	public void setVisited(){
		visited = true;
	}

	// Getter to check if square has been visited
